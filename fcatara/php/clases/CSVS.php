...
class CSVS {
  ...
  public function getDataTablas(){
		...
		$ftabla="../../datos_tablas/tabla_centro_provincia_naturaleza.html";
  ...
  public function makeQueryListados(){
			...
			if($dimorder=='enseñanza') $norder=' ORDER BY field(cod_enseñanza_agrupada,"INFANTIL","PRIMARIA","ESO","BACHILLERATO","BÁSICA","GRADO MEDIO","GRADO SUPERIOR","ARTÍSTICAS","IDIOMAS","ESPECIAL","ADULTOS","DEPORTIVAS"), '.$order;
			//else $order='';
			$select=$select.') FROM '.$this->tabla.' GROUP BY '.trim($groupby,",").' HAVING COUNT(*)>=1'.$norder.'  LIMIT '.$this->limit;
			$this->query=$select;
			//print($select);exit();
			return 1;
	}
