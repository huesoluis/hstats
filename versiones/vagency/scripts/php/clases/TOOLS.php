  ...
  public function load_custom_tabla($fcsvdata,$tabla)
	{
		...
		if (($gestor = fopen($fcsvdata, "r")) !== FALSE) 
		{
			while (($linea = fgetcsv($gestor, 0, "\n")) !== FALSE) 
			{
				$sql="INSERT INTO $tabla(curso,naturaleza,dengenerica,codcentro,nombrecentro,direccion,codpostal,provincia,municipio,nalumnos) VALUES(";
				//omitimos cabecera
				if($numero_registros==0)
				{
					$numero_registros++;
		...
		if (($gestor = fopen($fcsvdata, "r")) !== FALSE) 
			{
				...
				$reg=explode($sep,$linea[0]);
				$sql="INSERT INTO $tabla VALUES(0,";
				//print_r($reg);exit();
				foreach($reg as $r) 
				{
					$r=str_replace("'","",$r);
